<template>
    <div>
      <div class="bg_fon">
          ...
          <nuxt-link to="/category" class="bred" exact>Tag&nbsp;/&nbsp;</nuxt-link>
          <h1  v-for="(tag, index) in AllTags"
               :key="index">
            <span class="mt-2" v-if="tag.id == $route.params.tagid">
               {{ tag.title }}
            </span>
          </h1>
      ...
      <v-container class="my-10">
        ...
        <div v-else>
          ...
          <div v-else>
            <h3>Posts not found by this tag</h3>
          </div>
        </div>
      </v-container>
    </div>
  ...
  export default {
    ...
    //   tagPosts() {
    //   return this.posts.filter(elem => elem === this.tagid)
    // },
    ...
    methods: {
      ...mapActions({
        ...
        getTags: 'tags/getAllTags',
      }),
    },

    async mounted() {
      this.isloading = true;
      const res = await this.$axios.get(`/front/tag-posts/${this.$route.params.tagid}`);
