...
export default {
  ...
  methods: {
    ...mapActions({
      updateGallery: "galleries/updateGallery",
    }),
    onChangeFile(e) {
      this.new_image = e;
    },
    update() {
      if (!this.title) {
        ...
        formdata.append("id", this.$route.params.galleryid);
  ...
  async mounted(_) {
    ...
    const res = await axios.put(
      `http://localhost:8000/api/admin/galleries/${this.$route.params.galleryid}`,
      _,
      config
    );
