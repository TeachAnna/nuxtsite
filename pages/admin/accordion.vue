    ...
    <v-container>
      <v-card class="pa-3 mt-15 elevation-5">
        <v-card-text class="d-flex justify-space-between align-center">
          <v-card-title> Accordions ( {{ accordions.length }} ) </v-card-title>
          <v-card-actions>
            <nuxt-link to="/admin/accordions/create-accordion">
          ...
          <div v-else>
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="(accordion, index) in new_page_items" :key="index">
                    <td class="">{{ index + 1 }}</td>
                    <td class="">{{ accordion.title }}</td>
                    <td class="">
                      <v-img
                        :src="`${$store.state.urlSite}/storage/${accordion.image}`"
                        width="70"
                        cover
                      ></v-img>
                    </td>
                    <td class="">
                      <div v-if="accordion.status == 'true'">
                    ...
                    <td class="">
                      <nuxt-link
                        :to="`/admin/accordions/update-accordion/${accordion.id}`"
                      >
                    ...
                    <td class="">
                      <v-btn
                        @click="delAccordion(accordion.id)"
                        text
                        class="ma-2"
                        variant="text"
                        color="red"
        ...
        <v-card-actions class="justify-center pt-3 mb-5">
          <div class="pagination">
            ...
            <v-btn
              ...
              outlined
              class="active"
              v-if="accordions.length >= num_per_page"
              >{{ page + 1 }}</v-btn
            >
            <v-btn
              color="yellow lighten-2"
              ...
              outlined
              v-if="
                this.num_per_page * (this.page + 1) < this.accordions.length
              "
              v-on:click="set_page(+1)"
              >{{ page + 2 }}</v-btn
            >
            <v-btn
              ...
              outlined
              v-if="
                this.num_per_page * (this.page + 2) < this.accordions.length
              "
              v-on:click="set_page(+2)"
              >{{ page + 3 }}</v-btn
            >
...
export default {
  ...
  computed: {
    ...mapGetters({
      accordions: "accordions/getAccordions",
    }),
    new_page_items: function () {
      return this.accordions.slice(
  ...
  methods: {
    ...
    ...mapActions({
      getAccordions: "accordions/getAccordions",
      deleteAccordion: "accordions/deleteAccordion",
    }),
    delSlide(id) {
