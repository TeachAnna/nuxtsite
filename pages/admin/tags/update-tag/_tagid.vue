      ...
      <v-row justify="center" class="mt-10">
        <v-col>
          ...
          <v-card class="text-center pa-5 elevation-5">
            <v-card-title class="justify-center mb-7"> Update Tag</v-card-title>
            <v-card-subtitle>
...
export default {
  ...
  methods: {
    ...
    update() {
      if (!this.title) {
        ...
        formdata.append("id", this.$route.params.tagid);
  ...
  async mounted(_) {
    ...
    const res = await axios.put(
      `http://localhost:8000/api/admin/tags/${this.$route.params.tagid}`,
      _,
      config
    );
