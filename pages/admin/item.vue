    ...
    <v-container>
      <v-card class="pa-3 mt-15 elevation-5">
        <v-card-text class="d-flex justify-space-between align-center">
          <v-card-title> Items ( {{ items.length }} ) </v-card-title>
          <v-card-actions>
            <nuxt-link to="/admin/items/create-item">
          ...
          <div v-else>
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="(item, index) in new_page_items" :key="index">
                    <td class="">{{ index + 1 }}</td>
                    <td class="">{{ item.title }}</td>
                    <td class="">
                      <v-img
                        :src="`${$store.state.urlSite}/storage/${item.image}`"
                        width="70"
                        cover
                      ></v-img>
                    </td>
                    ...
                    <td class="">
                      <nuxt-link :to="`/admin/items/update-item/${item.id}`">
        ...
        <v-card-actions class="justify-center pt-3 mb-5">
          <div class="pagination">
            ...
            <v-btn
              ...
              outlined
              class="active"
              v-if="items.length >= num_per_page"
              >{{ page + 1 }}</v-btn
            >
            <v-btn
              color="yellow lighten-2"
              text
              outlined
              v-if="this.num_per_page * (this.page + 1) < this.items.length"
              v-on:click="set_page(+1)"
              >{{ page + 2 }}</v-btn
            >
            <v-btn
              ...
              text
              outlined
              v-if="this.num_per_page * (this.page + 2) < this.items.length"
              v-on:click="set_page(+2)"
              >{{ page + 3 }}</v-btn
            >
            <v-btn
