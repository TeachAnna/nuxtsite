          ...
          <div v-else>
            <v-simple-table>
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="(comment, index) in new_page_items" :key="index">
                    <td class="">{{ index + 1 }}</td>
                    <td class="">{{ comment.post_id }}</td>
                    <td class="">{{ comment.name }}</td>
                    <td class="">{{ comment.email }}</td>
                    <td class="">{{ comment.comment }}</td>
                    <td class="">
                      <div v-if="comment.status == 'true'">
        ...
        <v-card-actions class="justify-center pt-3 mb-5">
          <div class="pagination">
            ...
            <v-btn
              ...
              outlined
              class="active"
              v-if="comments.length >= num_per_page"
              >{{ page + 1 }}</v-btn
            >
            <v-btn
              color="yellow lighten-2"
              text
              outlined
              v-if="this.num_per_page * (this.page + 1) < this.comments.length"
              v-on:click="set_page(+1)"
              >{{ page + 2 }}</v-btn
            >
            <v-btn
              ...
              text
              outlined
              v-if="this.num_per_page * (this.page + 2) < this.comments.length"
              v-on:click="set_page(+2)"
              >{{ page + 3 }}</v-btn
            >
            <v-btn
...
export default {
  ...
  computed: {
    ...mapGetters({
      comments: "comments/getAdminComments",
    }),
  ...
  methods: {
    ...
    ...mapActions({
      getAdminsComments: "comments/getAdminsComments",
    }),
    ...mapActions({
